...
sub load_all_sentinel_sentinel_actions($self) {
    ...
    $self->sentinel_sentinel_actions->load_all_sentinel_sentinel_actions_p()->then(sub($result) {
        $self->render(json => $result->{data});
    })->catch(sub($err) {
        $self->app->log->error('Daje::Controller::Super::vSentinelSentinelActionsList::load_all_sentinel_sentinel_actions ' . $err);
        $self->render
    });
}
